...
def forward_file(sender_connection, data, users_table):
    try:
        ...
        while received_bytes < file_size:
            ...
            sys.stdout.write(f"\rMengirim file ke {recipient_username}: {loading_percentage}%")
            sys.stdout.flush()
        print(f"\nFile {file_name} successfull! {recipient_username}")
    except Exception as e:
        print(f"{_get_current_time()} File ERROR!!!: {str(e)}")
    finally:
        return
...
def setup_server():
    ...
    server_address = ('10.217.21.14', 6000)  # Ganti dengan alamat IP lokal mesin server
...
def _on_new_client(connection, users_table, users_last_message):
    try:
        ...
        print(f'{_get_current_time()} {client_name} joined the room chat !!')
